...
# --- FUNGSI UNTUK MEMUAT DATA DARI FILE EXCEL (VERSI FINAL) ---
@st.cache_data
def load_data_from_excel(path="data.xlsx"):
    ...
    try:
        ...
        df_prov = df_info_raw[['KLASTER', 'PROVINSI']].copy(); df_prov.rename(columns={'KLASTER': 'klaster', 'PROVINSI': 'pemda'}, inplace=True); df_prov['tingkat'] = 'provinsi'
...
# --- FUNGSI UNTUK MEMBUAT TAB ANALISIS (LOGIKA DIPERBAIKI) ---
def create_analysis_tab(level, info_df, parameter_df, kinerja_df, kondisi_df, stat_df):
    # Logika sidebar hanya muncul jika tabnya aktif
    ...
    # PERBAIKAN UTAMA: Membaca dari kolom 'INDEKS KINERJA' dan 'INDEKS KONDISI'
...
# PERBAIKAN TAMPILAN: Menggunakan st.columns untuk merapikan
with tab1:
